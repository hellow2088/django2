...
class BooksView(View):
    ...
    def get(self, request):
        logging.debug('当前类%s--当前路径方法%s',self.__class__.__name__,request)
    ...
    def post(self, request):
        ...
        if title is None:
            return JsonResponse({'error': '错误信息'}, status=400)
        book = Book.objects.create(title=title, press=press, date=date)
        return JsonResponse({
            'id': book.id,
            'title': book.title,
            'press': book.press,
...
def showbooks2(request):
    ...
    # 返回的是当前页的数据数量
    return render(request, 'booklist/showbooks2.html',
                  {'book': results, 'pageinfo': pageinfo, 'current_page': pageinfo.current_page})
